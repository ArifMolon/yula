...
- [x] Run `node scripts/verify-plan-html.mjs` and confirm it fails because `okf/plan.html` does not exist.
- [x] Create the semantic shell in `okf/plan.html`: skip link, header, sticky navigation, main sections, review drawer, review dialog, import input, toast region, and embedded CSS/JS boundaries.
...
- [x] Serve the workspace locally and open `okf/plan.html` in the browser.
...
- [x] Compare every major source heading and all phase exit criteria with `okf/YULA_DDD_Proje_Plani.md`; correct omissions before completion.
...
- [x] Stage `okf/plan.html`, `scripts/verify-plan-html.mjs`, this plan, and the approved design specification.
